...
class file:
    ...
    def set_file_id_offset(self, apfs, kdl):
        file_id_offset=[]
        for i in kdl.key_data_len:
            apfs.f.seek(apfs.EBT+0x38+kdl.size+i[0])
            offset=int.from_bytes(apfs.f.read(4)[::-1], byteorder='big')
            apfs.f.seek(apfs.EBT+apfs.block_size-0x28-i[2])
    ...
    def __init__(self, apfs, node_id_offset):
        kdl=keydatalen(apfs, apfs.EBT)
        self.set_file_id_offset(apfs, kdl)
        node_id=[]
        self.file_info=[]
